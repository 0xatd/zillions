...
export function createLivingWorldOperationsHandler(deps = {}) { const fetchImpl = deps.fetch || globalThis.fetch; return async function handler(req, res) { try {
  ...
  const snapshot = deps.snapshot ? await deps.snapshot() : await (async () => {
    ...
    const [leases, commands, tutorials, runtimeHealth, expectedActiveRegions, openEncounters, activeSieges, recentEvents] = await Promise.all([
    ...
    return { leases, commands, tutorials, runtimeHealth, expectedActiveRegions, openEncounters, activeSieges, recentEvents };
  })();
