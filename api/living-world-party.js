...
const ACTIONS = new Set(['create','invite','accept','decline','revoke','leave','travel_mode','group_travel','kick','promote']);
const KEYS = new Set(['requestId','action','partyId','targetId','targetHandle','inviteId','payload']);
const PAYLOAD_KEYS = { create: ['name','shardId'], travel_mode: ['mode'], group_travel: ['routeId','expectedRevisions'], promote: ['role'] };
...
export function validatePartyBody(body){
  ...
  if(body.targetHandle!=null&&!/^[a-z0-9_]{3,32}$/.test(body.targetHandle)) throw Object.assign(new Error('invalid_target_handle'),{status:400});
  ...
  return { requestId:body.requestId,action:body.action,partyId:body.partyId||null,targetId:body.targetId||null,targetHandle:body.targetHandle||null,inviteId:body.inviteId||null,payload };
}
