...
export function createLivingWorldWorkerHandler(deps={}){const fetchImpl=deps.fetch||globalThis.fetch;return async(req,res)=>{try{
  ...
  const results=[];
  for(const region of regions){let leaseEpoch=0,started=performance.now();try{const lease=deps.claim?await deps.claim(region.region_id,workerId):await rpc(config,'claim_world_region_lease',{p_region:region.region_id,p_worker:workerId,p_lease_seconds:120},fetchImpl);if(lease?.ok===false||lease?.status==='lease_held'){results.push({regionId:region.region_id,status:'lease_held'});continue;}leaseEpoch=Number(lease.leaseEpoch);const result=deps.process?await deps.process(region.region_id,workerId,leaseEpoch):await rpc(config,'process_world_region_runtime',{p_region:region.region_id,p_worker:workerId,p_lease_epoch:leaseEpoch,p_command_limit:100},fetchImpl);const duration=Math.max(0,performance.now()-started);const saturated=Number(result?.population?.present)>Number(result.actionBudget)&&Number(result?.factions?.processed)>=Number(result.actionBudget);const record=deps.record?await deps.record(region.region_id,result.tick,workerId,leaseEpoch,duration,true,null,result.actionBudget,saturated):await rpc(config,'record_world_region_runtime_health',{p_region:region.region_id,p_world_tick:result.tick,p_worker:workerId,p_lease_epoch:leaseEpoch,p_duration_ms:duration,p_success:true,p_error_code:null,p_action_budget:result.actionBudget,p_action_saturated:saturated},fetchImpl);results.push({regionId:region.region_id,status:'advanced',result,health:record});}catch(error){const duration=Math.max(0,performance.now()-started);try{if(leaseEpoch>0){if(deps.record)await deps.record(region.region_id,0,workerId,leaseEpoch,duration,false,'region_runtime_failed',0,false);else await rpc(config,'record_world_region_runtime_health',{p_region:region.region_id,p_world_tick:0,p_worker:workerId,p_lease_epoch:leaseEpoch,p_duration_ms:duration,p_success:false,p_error_code:'region_runtime_failed',p_action_budget:0,p_action_saturated:false},fetchImpl);}}catch{}results.push({regionId:region.region_id,status:'failed',error:error.message});}}
  return send(res,results.some(row=>row.status==='failed')?207:200,{ok:!results.some(row=>row.status==='failed'),workerId,batchLimit:regionLimit,regions:results});
