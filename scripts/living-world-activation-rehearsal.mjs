...
try{
  ...
  const names=(await readdir(path.join(root,'supabase/migrations'))).filter(name=>name.endsWith('.sql')).sort();
  const migrationSql=await Promise.all(names.map(async(name)=>({name,sql:await readFile(path.join(root,'supabase/migrations',name),'utf8')})));
  const transactionBody=(sql)=>sql.replace(/^\s*begin;\s*/i,'').replace(/\s*commit;\s*$/i,'');
  // Rehearse the complete living-world stack as one activation transaction.
  // Any failure must return the database to its exact pre-activation schema.
  await admin.query('begin');
  for(const migration of migrationSql)await admin.query(transactionBody(migration.sql));
  ...
  assert.equal((await admin.query("select to_regclass('public.world_shards') name")).rows[0].name,null,'full rollback must remove the first living-world migration');
  assert.equal((await admin.query("select to_regclass('public.world_region_runtime_ticks') name")).rows[0].name,null,'full rollback must remove the final living-world migration');
  // Apply normally after the rollback to prove the same artifact activates cleanly.
  for(const migration of migrationSql)await admin.query(migration.sql);
  await admin.query("select set_config('request.jwt.claim.role','service_role',false)");
  assert.equal((await admin.query("select to_regclass('public.world_region_runtime_ticks') name")).rows[0].name,'world_region_runtime_ticks');
  console.log(`living world full-stack activation rollback rehearsal passed (${migrationSql.length} migrations)`);
