...
assert.match(sql,/hashtextextended\(v_party\.id::text,0\)\+v_state\.simulation_tick/,'goal selection must derive from stable identity and authoritative tick');
...
assert.match(sql,/world_parties p[\s\S]*p\.region_id=p_region and p\.owner_user_id is null/,'only server-owned parties in the leased region may be simulated');
