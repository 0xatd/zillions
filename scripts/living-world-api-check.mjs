...
assert.deepEqual(crossingProjection.routes.map((row)=>row.id),['cross-region']);assert.deepEqual(crossingProjection.parties.map((row)=>row.id),['mine']);assert.ok(crossingProjection.locations.some((row)=>row.id==='adjacent'),'loader must retain minimal hostile cross-region endpoint geometry without exposing settlement details');assert.equal(crossingProjection.locations.find((row)=>row.id==='adjacent').services,undefined);
