...
const processRegion=async(region,worker,epoch)=>{const started=performance.now();try{const result=(await admin.query('select public.process_world_region_runtime($1,$2,$3,100) result',[region,worker,epoch])).rows[0].result;const duration=performance.now()-started;const saturated=Number(result.population.present)>Number(result.actionBudget)&&Number(result.factions.processed)>=Number(result.actionBudget);const health=(await admin.query('select public.record_world_region_runtime_health($1,$2,$3,$4,$5,true,null,$6,$7) result',[region,result.tick,worker,epoch,duration,result.actionBudget,saturated])).rows[0].result;return{result,duration,health};}catch(error){const duration=performance.now()-started;await admin.query("select public.record_world_region_runtime_health($1,0,$2,$3,$4,false,'region_runtime_failed',0,false)",[region,worker,epoch,duration]).catch(()=>{});throw error;}};
try{
  ...
  const conservedFoodBefore=Number((await admin.query("select coalesce((select sum(stock) from public.world_markets where commodity_key='food'),0)+coalesce((select sum(quantity) from public.world_cargo where commodity_key='food'),0) total")).rows[0].total);
  ...
  const canonicalPatrol=(await admin.query("select id from public.world_parties where home_region_id=$1 and strategic_role='patrol'",[densityRegion])).rows[0].id;
  await admin.query("update public.world_parties set strategic_role='scout' where id=$1",[canonicalPatrol]);
  await admin.query('select public.reconcile_world_region_population($1,2)',[densityRegion]);
  assert.deepEqual(Object.fromEntries((await admin.query('select strategic_role,count(*)::integer count from public.world_parties where home_region_id=$1 group by strategic_role order by strategic_role',[densityRegion])).rows.map(row=>[row.strategic_role,Number(row.count)])),{caravan:1,garrison:1,patrol:1,raider:1,scout:1,siege_force:1},'role reconciliation must repair a corrupted canonical role');
  ...
  // Present-region load is independent of home population. Prove overload
  // classification, the adaptive capped budget, and a persisted breach.
  const probeHome=regions[1];
  await admin.query(`insert into public.world_parties(id,shard_id,region_id,home_region_id,owner_faction_id,name,kind,location_id,strategic_role)
    select md5('phase2-hot-probe:'||g)::uuid,'earth-1',$1,$2,p.owner_faction_id,'Hot Probe '||g,'ai',l.id,'patrol'
    from generate_series(1,11) g cross join public.world_provinces p join public.world_locations l on l.province_id=$1 and l.is_region_seat where p.id=$1`,[densityRegion,probeHome]);
  const hot=await processRegion(densityRegion,'phase2-soak',leases.get(densityRegion));
  assert.equal(hot.result.population.congestion,'overloaded');assert.equal(Number(hot.result.actionBudget),16);assert.equal(hot.health.thresholdBreached,true);
  await admin.query("delete from public.world_parties where name like 'Hot Probe %'");

  const latencies=[];let errors=0;const started=performance.now();
  for(let round=0;round<56;round++)for(const region of regions){if(round===0&&region===densityRegion)continue;try{
    const tick=await processRegion(region,'phase2-soak',leases.get(region));latencies.push(tick.duration);
    assert.ok(Number(tick.result.actionBudget)<=16);assert.equal(tick.result.population.state,'healthy');
  ...
  assert.equal(errors,0);assert.ok(p95<250,`region runtime p95 ${p95.toFixed(1)}ms exceeds 250ms`);assert.ok(elapsed<60000,`56-tick soak ${elapsed.toFixed(1)}ms exceeds 60s`);
  ...
  assert.ok(Number((await admin.query("select count(*) count from public.world_pursuits where state in('caught','escaped') and resolved_tick is not null")).rows[0].count)>0,'pursuit lifecycle did not resolve');
  assert.ok(Number((await admin.query("select count(*) count from public.world_pursuits where result ? 'chaseRouteId' and result ? 'targetLocationId'")).rows[0].count)>0,'pursuits did not drive target-aware chase movement');
  assert.ok(Number((await admin.query("select count(*) count from public.world_sieges where status in('resolved','lifted')")).rows[0].count)>0,'siege lifecycle did not resolve');
  assert.ok(Number((await admin.query("select count(*) count from public.world_siege_advances")).rows[0].count)>0,'sieges were never advanced');
  assert.ok(Number((await admin.query("select count(*) count from public.world_raid_orders where state='resolved' and result ? 'success'")).rows[0].count)>0,'raid lifecycle did not resolve');
  assert.ok(Number((await admin.query("select count(*) count from public.world_caravan_plans where state='buying' and revision>=4")).rows[0].count)>0,'no caravan completed an outbound and return cycle');
  const conservedFoodAfter=Number((await admin.query("select coalesce((select sum(stock) from public.world_markets where commodity_key='food'),0)+coalesce((select sum(quantity) from public.world_cargo where commodity_key='food'),0) total")).rows[0].total);
  assert.equal(conservedFoodAfter,conservedFoodBefore,'caravan and raid logistics must conserve food across markets and cargo');
  assert.ok(Number((await admin.query("select count(*) count from public.world_region_runtime_health where success")).rows[0].count)>=56*72,'runtime health rows missing');
  assert.equal(Number((await admin.query("select count(*) count from public.world_region_runtime_health where not success")).rows[0].count),0);
  assert.ok(Number((await admin.query("select count(*) count from public.world_region_runtime_health where threshold_breached and congestion_state='overloaded'")).rows[0].count)>=1,'overload breach was not persisted');
  ...
  await processRegion(takeoverRegion,'phase2-takeover',Number(takeover.leaseEpoch));
  console.log(`Phase 2 PostgreSQL soak passed: ${total} parties, 56x72 ticks, p95 ${p95.toFixed(1)}ms, max ${max.toFixed(1)}ms, ${elapsed.toFixed(1)}ms total`);
