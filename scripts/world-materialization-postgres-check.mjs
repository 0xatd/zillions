...
try{
  ...
  assert.equal((await admin.query("select count(*) count from public.world_parties p join public.world_armies a on a.party_id=p.id left join public.world_unit_stacks s on s.army_id=a.id where p.owner_user_id is null and s.id is null")).rows[0].count,'0','every ownerless strategic army must have a tactical stack');
