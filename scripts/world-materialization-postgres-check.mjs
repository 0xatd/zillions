...
const postgres=new EmbeddedPostgres({databaseDir,port:30000+Math.floor(Math.random()*1000),user:'postgres',password:'postgres',persistent:false,onLog(){}});let admin,aiTacticalMigration;
...
try{
  ...
  for(const name of(await readdir(path.join(root,'supabase/migrations'))).filter(name=>name.endsWith('.sql')).sort()){const sql=await readFile(path.join(root,'supabase/migrations',name),'utf8');if(name==='20260821100000_ai_tactical_force_seed.sql'){aiTacticalMigration=sql;continue;}await admin.query(sql);}
  ...
  assert.equal((await admin.query('select count(*) count from public.world_unit_stacks')).rows[0].count,'0','the pre-fix production schema must reproduce missing AI tactical stacks');
  await admin.query(aiTacticalMigration);
  const isolatedPlayer='90000000-0000-4000-a000-000000000003';await admin.query("insert into public.world_parties(id,shard_id,region_id,name,kind,location_id) select $1,'earth-1',p.id,'Orphan Player','player',l.id from public.world_provinces p join public.world_locations l on l.province_id=p.id order by p.id,l.id limit 1",[isolatedPlayer]);await admin.query('insert into public.world_armies(party_id,commander_user_id,combat_power) values($1,$2,10)',[isolatedPlayer,user]);assert.equal((await admin.query('select count(*) count from public.world_unit_stacks s join public.world_armies a on a.id=s.army_id where a.party_id=$1',[isolatedPlayer])).rows[0].count,'0','forward trigger must not classify a null-owner player as AI');await admin.query('delete from public.world_parties where id=$1',[isolatedPlayer]);
  ...
  assert.equal((await admin.query("select count(*) count from public.world_parties p join public.world_armies a on a.party_id=p.id left join public.world_unit_stacks s on s.army_id=a.id and s.healthy>0 where p.owner_user_id is null and a.commander_user_id is null and p.kind in('ai','caravan','patrol','garrison') and a.combat_power>0 and s.id is null")).rows[0].count,'0','every live AI strategic army must have positive tactical strength');
  ...
  await expectError(admin.query("insert into public.world_provinces(id,shard_id,key,name,bounds,planet_id) values('99999999-0000-4000-8000-000000000001','earth-1','extra','Extra','{}','earth')"),'materialized_world_topology_immutable');
  await expectError(admin.query("insert into public.world_locations(id,province_id,key,name,kind,position) select '99999999-0000-4000-8000-000000000002',id,'extra','Extra','town','{}' from public.world_provinces where planet_id='earth' order by id limit 1"),'materialized_world_topology_immutable');
  await expectError(admin.query("insert into public.world_routes(id,province_id,origin_id,destination_id,distance,origin_region_id,destination_region_id) select '99999999-0000-4000-8000-000000000003',o.province_id,o.id,d.id,1,o.province_id,d.province_id from public.world_locations o join public.world_locations d on d.id<>o.id where o.province_id in(select id from public.world_provinces where planet_id='earth') order by o.id,d.id limit 1"),'materialized_world_topology_immutable');
  // Rows cannot evade the ready-world fence by entering Earth through UPDATE.
  await admin.query("insert into public.world_planets(id,system_id,key,name) select 'pending-planet',system_id,'pending-planet','Pending Planet' from public.world_planets where id='earth'");
  await admin.query("insert into public.world_provinces(id,shard_id,key,name,bounds,planet_id) values('99999999-0000-4000-8000-000000000011','earth-1','pending','Pending','{}','pending-planet')");
  await admin.query("insert into public.world_locations(id,province_id,key,name,kind,position) values('99999999-0000-4000-8000-000000000012','99999999-0000-4000-8000-000000000011','pending-a','Pending A','town','{}'),('99999999-0000-4000-8000-000000000013','99999999-0000-4000-8000-000000000011','pending-b','Pending B','town','{}')");
  await admin.query("insert into public.world_routes(id,province_id,origin_id,destination_id,distance,origin_region_id,destination_region_id) values('99999999-0000-4000-8000-000000000014','99999999-0000-4000-8000-000000000011','99999999-0000-4000-8000-000000000012','99999999-0000-4000-8000-000000000013',1,'99999999-0000-4000-8000-000000000011','99999999-0000-4000-8000-000000000011')");
  await expectError(admin.query("update public.world_provinces set planet_id='earth' where id='99999999-0000-4000-8000-000000000011'"),'materialized_world_topology_immutable');
  await expectError(admin.query("update public.world_locations set province_id=(select id from public.world_provinces where planet_id='earth' order by id limit 1) where id='99999999-0000-4000-8000-000000000012'"),'materialized_world_topology_immutable');
  await expectError(admin.query("update public.world_routes set province_id=(select id from public.world_provinces where planet_id='earth' order by id limit 1),origin_region_id=(select id from public.world_provinces where planet_id='earth' order by id limit 1) where id='99999999-0000-4000-8000-000000000014'"),'materialized_world_topology_immutable');
