...
export function summarizeWorldOperations(snapshot = {}, now = Date.now()) {
  const leases = snapshot.leases || [], commands = snapshot.commands || [], tutorials = snapshot.tutorials || [];
  ...
  return { status: staleLeases.length || stuckCommands.length ? 'degraded' : 'healthy', activeRegions: leases.length - staleLeases.length,
