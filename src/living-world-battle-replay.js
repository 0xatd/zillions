...
import { validateBattleResult } from './living-world-battle.js';
...
export function verifyLivingWorldBattleReplay(assignment,input){
  ...
  const canonical={assignmentId:assignment.id,completedTick:tick,outcome,winnerPartyId,casualties,morale:{attacker:attackerMorale,defender:defenderMorale}};
  return validateBattleResult({outcome,winnerPartyId,casualties,morale:canonical.morale,cargoTransfers:[],prisoners:[],retreatRoutes:[],completedTick:tick,stateHash:createHash('sha256').update(JSON.stringify(canonical)).digest('hex')});
}
