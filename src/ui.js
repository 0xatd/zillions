...
export class UI {
  ...
  _renderLivingWorldMap() {
    ...
    encounterBox.innerHTML = state.encounters.length ? state.encounters.map((encounter) => `<div class="lw-selection"><small>${escapeHtml(encounter.state.toUpperCase())} ENCOUNTER</small><b>${escapeHtml(encounter.ownSide === 'attacker' ? 'You caught a hostile force' : 'A hostile force caught you')}</b><p>Terrain: ${escapeHtml(JSON.stringify(encounter.terrain || {}))}</p><div class="lw-company-actions">${encounter.engagement ? '<button class="tbtn" data-encounter="autosim" data-id="'+escapeHtml(encounter.id)+'">AUTO-COMMAND</button>' : ['fight','escape','surrender','parley','rearguard'].map((choice) => `<button class="tbtn" data-encounter="${choice}" data-id="${escapeHtml(encounter.id)}">${choice.toUpperCase()}</button>`).join('')}</div></div>`).join('') : '';
