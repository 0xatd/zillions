...
class App {
  ...
  _startTutorial() {
    const steps = [
      ...
      [14, '💰 Walk to a glowing foundation and HOLD SPACE — your coins build it. Income is paid automatically; coins drop from fighting.'],
      ...
      [62, '🔧 Nothing repairs itself — hold SPACE on damaged or ruined buildings to fix them. Press T beside a tower to change what it shoots.'],
    ];
    this._tut = { steps, i: 0 };
  }
      ...
      if (mh && !mh.dead && this.game.phase === 'live' && !branchPlot) {
        ...
        if (target) {
          ...
          hint = mh.payHold
            ...
            : `<div>Hold <kbd>SPACE</kbd> — ${verb} <b>${name}</b> (${cost}🪙)</div><div class="buildrole">${role}</div>`;
        }
      }
