...
class App {
  ...
  async _openLobby() {
    ...
    this.lobby = new OnlineLobby({
      ...
      onGames: (g) => this.ui.lobbyGames(g, (row) => this.joinOnlineGame(row), this.lobby?.me?.id),
  ...
  // both ways through the signaling channel afterwards, so the connection
  // comes up fast and settles on the best route instead of freezing whatever
  // was gathered in the first seconds.
  async _onSignal(sig) {
    if (sig.t === 'offer' && this.mpRole === 'guest') {
      const wasInGame = this.netMode && this.game && !this.game.over;
      ...
      this.net.onOpen = () => {
        ...
        // Mid-game reconnects wait for the host's resync instead.
      };
      this.net.onMessage = (m) => this._onGuestMsg(m);
