...
export class Game {
  ...
  snapshot() {
    return {
      ...
      time: +this.time.toFixed(3), threat: +this.threat.toFixed(4), threatLevel: this.threatLevel,
      ...
      plots: this.plots.map((p) => ({
        id: p.id, tier: p.tier, paid: +p.paid.toFixed(3), branch: p.branch,
        ruined: p.ruined ? 1 : 0, musterT: p.musterT != null ? +p.musterT.toFixed(2) : null,
      })),
      ...
      units: this.units.filter((u) => !u.hero).map((u) => ({
        ...
        hp: +u.hp.toFixed(2), camp: u.camp || 0, node: u.targetNodeId != null ? u.targetNodeId : -1,
  ...
  _restore(snap) {
    ...
    this.relicMods = itemMods(this.relics);
    (snap.nests || []).forEach(([hp, alive, musterT], i) => {
      if (this.nests[i]) {
        this.nests[i].hp = hp;
